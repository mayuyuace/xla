...
absl::StatusOr<std::unique_ptr<Kernel>> GpuExecutor::LoadKernel(
  ...
  auto l0_kernel = std::make_unique<GpuKernel>(this);
  ...
  if (spec.has_cuda_cubin_in_memory()) {
    ...
    int size = spec.cuda_cubin_in_memory().cubin_bytes().size();
    absl::MutexLock lock{&in_memory_modules_mu_};
    TF_RETURN_IF_ERROR(LoadModuleFromSpir(spirv, size, &module));
    kernel_to_gpu_binary_[l0_kernel.get()] = spirv;
  } else {
    return absl::Status(
  ...
  l0_kernel->set_gpu_function(function);
  ...
  TF_RETURN_IF_ERROR(GetKernelMetadata(l0_kernel.get(), &kernel_metadata));
  l0_kernel->set_metadata(kernel_metadata);
  l0_kernel->set_name(kernel_name);
  return std::move(l0_kernel);
}
...
absl::Status GpuExecutor::Launch(Stream* stream, const ThreadDim& thread_dims,
  ...
  GpuFunctionHandle sycl_kernel = spir_kernel->gpu_function();
